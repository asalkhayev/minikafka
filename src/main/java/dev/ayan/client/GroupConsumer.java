...
public class GroupConsumer implements AutoCloseable {
    ...
    public GroupConsumer(String host, int port, String groupId, String topicName, int partitionId) throws IOException {
        ...
        validateAndEncodeGroupId(groupId);
        ...
        System.out.println("[GroupConsumer] Started group=" + groupId +
                ", topic=" + topicName +
    ...
    public synchronized List<LogEntry> poll(int maxMessages) throws IOException {
        ...
        if (responseCode != Protocol.RES_OK) {
            throw new IOException("Broker rejected FETCH request for group consumer " + groupId);
        }
        ...
        entries.forEach(entry ->
                System.out.println("[GroupConsumer] group=" + groupId +
                        " read " + topicName + "-" + partitionId +
    ...
    public synchronized void commit() throws IOException {
        byte[] groupBytes = validateAndEncodeGroupId(groupId);
        byte[] topicBytes = validateAndEncodeTopic(topicName);

        out.writeByte(Protocol.CMD_COMMIT_OFFSET);
        ...
        byte responseCode = in.readByte();
        if (responseCode != Protocol.RES_OK) {
            throw new IOException("Broker rejected COMMIT_OFFSET request for group " + groupId);
        }

        System.out.println("[GroupConsumer] Committed offset " + currentOffset +
                " for group=" + groupId +
                ", topic=" + topicName +
                ", partition=" + partitionId);
    }

    public synchronized long fetchCommittedOffset() throws IOException {
        byte[] groupBytes = validateAndEncodeGroupId(groupId);
        byte[] topicBytes = validateAndEncodeTopic(topicName);

        out.writeByte(Protocol.CMD_GET_OFFSET);
        out.writeShort(groupBytes.length);
        out.write(groupBytes);
        out.writeShort(topicBytes.length);
        out.write(topicBytes);
        ...
        byte responseCode = in.readByte();
        if (responseCode != Protocol.RES_OK) {
            throw new IOException("Broker rejected GET_OFFSET request for group " + groupId);
        }

        long committedOffset = in.readLong();
        if (committedOffset < 0) {
            throw new IOException("Broker returned invalid committed offset: " + committedOffset);
        }
    ...
    private byte[] validateAndEncodeGroupId(String groupId) {
        ...
        if (groupBytes.length > Protocol.MAX_GROUP_ID_BYTES) {
            throw new IllegalArgumentException("Group id is too long: " + groupBytes.length + " bytes");
        }
        return groupBytes;
    }
